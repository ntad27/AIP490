...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in ['paired']:
            ...
            c[key] = transforms.Resize(self.fine_width, interpolation=InterpolationMode.BILINEAR)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key]))
            cm[key] = transforms.Resize(self.fine_width, interpolation=InterpolationMode.NEAREST)(cm[key])
        ...
        im_pil = transforms.Resize(self.fine_width, interpolation=InterpolationMode.BILINEAR)(im_pil_big)
        ...
        im_parse_pil = transforms.Resize(self.fine_width, interpolation=InterpolationMode.NEAREST)(im_parse_pil_big)
...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im = transforms.Resize(self.fine_width, interpolation=InterpolationMode.BILINEAR)(im)
        im = self.transform(im)

        # load parsing image
        parse_name = im_name.replace('.jpg', '.png')
        im_parse = Image.open(osp.join(self.data_path, 'image-parse-v3', parse_name))
        im_parse = transforms.Resize(self.fine_width, interpolation=InterpolationMode.NEAREST)(im_parse)
